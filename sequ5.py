...
#generateRange is function that I use in order to create a list containing each value that needs to be output. It works for both floats and ints
def generateRange(start, stop, increment):
    if(increment > 0):
            while start <= stop:
                yield start
                start += increment
...
#on each member of seqRange. Finally it returns this new list.
def createSeq(first, last, increment, layout, sep):
    
    seqRange = list(generateRange(first, last, increment))
